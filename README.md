...
It is written in Python against GTK4/libadwaita, and costs about 1.5 MB of disk and nothing resident: the GTK stack is already installed, and so is the interpreter, for `nixos-rebuild-ng` behind `system-upgrade`. What Python costs instead is start-up, so the bytecode is compiled into the store (a read-only directory never gets a `__pycache__` of its own) and the pages are built on first visit rather than all at once — together about a fifth off the time to a first window on a fast machine, and half a second or so on the hardware this targets, where the interpreter and the GTK typelibs are the rest of it. `features.settingsApp = false` removes the menu entry, the polkit action and that 1.5 MB.
